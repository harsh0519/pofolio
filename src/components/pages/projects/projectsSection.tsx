      ...
      <div className="relative z-10 max-w-7xl mx-auto">
        ...
        <div className="text-center mt-16">
          <a
            ...
            className="inline-flex items-center gap-2 px-8 py-4 glass-effect rounded-lg font-semibold text-[var(--text-primary)] transition-all duration-300 hover:scale-105 hover:border-[var(--accent-primary)]"
          >
...
function ProjectCard({
  ...
  return (
    <div
      className={`group relative glass-effect rounded-xl overflow-hidden transition-all duration-300 hover:scale-105 hover:border-[var(--accent-primary)] animate-fade-in-up ${
    ...
    >
      {/* Featured Badge */}
      {project.featured && (
        <div className="absolute top-4 right-4 z-10 px-3 py-1 bg-white text-black rounded-full text-xs font-bold">
          Featured
        </div>
      )}

      {/* Project Image/Icon */}
      <div className="relative h-48 bg-gradient-to-br from-gray-800/30 to-gray-600/30 flex items-center justify-center text-6xl overflow-hidden">
        {project.image}
        <div
          className={`absolute inset-0 bg-gradient-to-t from-[var(--bg-secondary)] to-transparent transition-opacity duration-300 ${
      ...
      {/* Project Info */}
      <div className="p-6">
        <h3 className="text-xl font-bold mb-2 text-[var(--text-primary)] group-hover:gradient-text transition-all">
          {project.title}
        </h3>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tags.map((tag, i) => (
            <span
              key={i}
              className="px-3 py-1 text-xs font-medium bg-[var(--bg-tertiary)] text-[var(--text-secondary)] rounded-full"
            >
              {tag}
            </span>
          ))}
        ...
        <div className="flex gap-3">
          {project.github && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2 glass-effect rounded-lg text-sm font-medium text-[var(--text-secondary)] hover:text-[var(--accent-primary)] hover:border-[var(--accent-primary)] transition-all"
            >
          ...
          {project.demo && (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="flex-1 flex items-center justify-center gap-2 px-4 py-2 bg-white text-black rounded-lg text-sm font-medium hover:scale-105 transition-all"
            >
